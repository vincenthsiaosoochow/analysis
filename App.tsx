            ...
            <div className="animate-fade-in max-w-md mx-auto min-h-screen p-8">
              ...
              <div className="space-y-4">
                {!isLoggedIn ? (
                  ...
                  <>
                    {/* Emergency Fix Button for specific user if not admin */}
                    {profile.phone === '13218185056' && !profile.is_admin && (
                      <button
                        onClick={async () => {
                          try {
                            if (window.confirm('确定要修复管理员权限吗？')) {
                        ...
                        className="w-full mb-4 bg-amber-500 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg"
                      >
