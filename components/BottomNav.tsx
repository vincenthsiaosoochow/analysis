...
const BottomNav: React.FC<BottomNavProps> = ({ currentTab, onTabChange }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-100 px-12 pt-3 pb-8 flex items-center justify-between z-50 max-w-md mx-auto">
      {tabs.map((tab) => (
        <button
          key={tab.id}
