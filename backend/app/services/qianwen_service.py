...
def analyze_artwork_with_qianwen(image_base64: str) -> Dict[str, Any]:
    ...
    # 构建分析提示词
    system_prompt = """你是拥有20年艺术史研究+10年艺术品投资顾问经验的专家，现对这幅画作进行**专业、结构化、可落地的分析**。
你的回复必须严格遵循 JSON 格式。所有文本应使用流畅、专业的中文。
在分析时，请结合画面细节展开，禁止空泛表述。使用专业术语且逻辑清晰。"""
...
请用流畅的中文输出，数据标注"参考值""近3年"等限定词，避免绝对化表述。请严格按照以下 JSON 格式返回：
{
  "title": "作品标题",
  "artist": "艺术家姓名",
  "artistGender": "男/女/未知",
  ...
  "composition": "构图描述（简要）",
  "interpretation": "作品解读（简要）",
  "coreAnalysis": {
    "styleAndSchool": "艺术风格与流派分析（含流派特征、契合点）",
    "colorUsage": "色彩运用分析（含主色调、对比色、情绪传递）",
    "brushworkTexture": "笔触与肌理分析（含技法、质感、体验）",
    "compositionLayout": "构图布局分析（含重心、层次、创新点）",
    "themeAndMood": "题材与意境分析（含具象/抽象、文化内涵）",
    "artisticValue": "艺术价值分析（含创新点、历史定位）"
  },
  "artistInfo": {
    "basics": "艺术家基础信息（姓名、国籍、生卒年、生涯阶段）",
    "marketPosition": "市场定位（梯队级别：一线/二线、蓝筹/成长型）",
    "representativeWorks": "代表作品及参考价（3-5幅核心作+价格区间）",
    "styleEvolution": "风格演变（各阶段特征+本画所属阶段认可度）"
  },
  "investmentAnalysis": {
    "rating": "S/A/B/C/D",
    "ratingReason": "评级依据（结合艺术特征+市场表现）",
    "marketTrends": "市场行情（近3年成交均价、成交量、涨跌趋势）",
    "collectionAdvice": "收藏建议（适合人群+配置比例+持有周期）",
    "riskAlert": "风险提示（核心风险点+规避建议）",
    "alternatives": "同类替代标的（2-3个艺术家+核心优势）"
  }
}"""
